    ...
    <div class="lg:h-10 h-auto lg:flex bg-color lg:items-center px-4 lg:justify-between my-5">
        <div class="flex items-center justify-between">
            <div>
                <nuxt-link :to="localePath('/')">
                    <span class="font-bold text-slate-800 text-color ">{{ $t('header.frame') }}</span><span class=" text-slate-500">{{ $t('header.projectName') }}</span>
                </nuxt-link>
            </div>
        ...
        <div class="lg:flex lg:items-center hidden lg:gap-6 flex-col lg:flex-row">
            <nuxt-link :to="localePath(page.path)" v-for="(page, index) in menuItems" :key="index">
                <span class="block text-color h-10 leading-10 text-gray-500 hover:text-gray-900 dark:hover:text-slate-500">{{ $t(`header.${page.name}`) }}</span>
            </nuxt-link>
            <div class="lg:hidden flex gap-3">
                ...
                <div
                    class="flex-1 cursor-pointer rounded-md h-10 leading-10 text-white text-center hover:bg-slate-700">{{ $t('btn.signup') }}</div>
            </div>
        </div>
        ...
        <div class="lg:flex items-center gap-2 hidden">
            ...
            <div class="cursor-pointer" @click="changeLang">{{ locale=='en'?'中文':'English' }}</div>
            <div>{{ $t('btn.login') }}</div>
            <div class="bg-btn cursor-pointer rounded-md h-10 leading-10 px-5 bg-text-color hover:bg-slate-700 hover:text-white">{{ $t('btn.signup') }}
            </div>
        </div>
    </div>
</template>
